...
import {
  ...
  Users, DollarSign, AlertTriangle, FileText, LogOut, LogIn, ShieldCheck, Upload
} from 'lucide-react'
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-slate-900 flex flex-col z-40">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto scrollbar-thin">
        {navItems.filter((it) => it.href !== '/import' || user).map(({ href, label, icon: Icon }) => {
      ...
      <div className="px-3 py-4 border-t border-slate-800">
        {user ? (
          ...
          <Link href="/login"
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 transition-colors w-full"
          >
